...
const Home = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const response = await axios.post('http://192.168.100.110:4500/login/obtenerUsuario', { userId: sessionStorage.getItem('userId') });
        ...
        if (ufAsociada) {
          const segundoEndpointResponse = await axios.get(`http://192.168.100.110:4500/UF/obtenerTodo/${ufAsociada}`);
